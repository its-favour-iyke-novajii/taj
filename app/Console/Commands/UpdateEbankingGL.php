...
class UpdateEbankingGL extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ctr:update-ebanking-gl {days}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $query = "SELECT trim(val1) FROM ctl_collections where name = 'ebanking_accounts'";

            // Execute the query
            $stmt = $p_conn->query($query);

            // Fetch all the val1 values and store them in an array
            $val1_values = $stmt->fetchAll(PDO::FETCH_COLUMN);

            // Enclose each value in parentheses and add single quotes around each value
            $val1_values_parentheses = array_map(function ($val) {
            return "'$val'";
            }, $val1_values);

            // Convert the array to a comma-separated string
            $comma_separated_values = implode(",", $val1_values_parentheses);

            // Now you can use $comma_separated_values as needed
            //echo "( $comma_separated_values )";
            $ctl_collections = "( $comma_separated_values )";
            ...
            $sql = 'SELECT trim(b.ncp) account_number, nvl(b.txind, 1) exchange_rate, trim(b.age) branch_code, trim(b.dev) currency_code, trim(b.dco) tran_date, '
                ...
                . " FROM tajprod.bksld b WHERE trim(b.ncp) in $ctl_collections and b.dco = trunc(sysdate) - $days";
            ...
            echo "Exchange Rate: $exchange_rate" . PHP_EOL; //
                ...
                try {
                  /*  $dsn = "pgsql:host=127.0.0.1;dbname=tajbank";
                    ...
                    // Prepare the SQL query
                    $query = "SELECT val1 FROM ctl_collections";
